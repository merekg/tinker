...
def parse_spreadsheet_line(line):

    navigation_point  = line[2:5]
    measured_volume_point = line[5:8]
    post_detector_tx = line[15:31].reshape((4,4))
    inverse_post_detector_tx = line[31:47].reshape((4,4))
    inverse_ground_truth_detector_tx = line[47:63].reshape((4,4))
    detector_tx = line[63:].reshape((4,4))

    calculated_volume_point = apply_rigid_transform(apply_rigid_transform(apply_rigid_transform(apply_rigid_transform(measured_volume_point, inverse_post_detector_tx), inverse_ground_truth_detector_tx), detector_tx), post_detector_tx)
    print(*list(calculated_volume_point))

def main():
    for line in open(sys.argv[1]):
        parse_spreadsheet_line(np.fromstring(line,sep=' '))
