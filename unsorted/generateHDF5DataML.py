...
class CreateHDF5:
    ...
    def createHDF5(self, fileName, reconRow, procedureRow):
        ...
        directionsData = np.zeros((3,3), int)
        np.fill_diagonal(directionsData, 1.0)
        directionsDataset = zeroGroup.create_dataset('Directions', directionsData.shape, dtype = int)
        ...
        vertebraeIdentifiedRaw = reconRow['vertebraeIdentified']
        vertebraeIdentified = vertebraeIdentifiedRaw[1:-1]
        ...
        with open(reconRow['savePath'], 'rb') as f:
            ...
            while byte != b"":
                ...
                byte = f.read(1)
        voxel = np.asarray(voxelArray, dtype=np.int32)
        voxelData = np.reshape(voxel, (int(reconRow['dimensionX']), int(reconRow['dimensionY']), int(reconRow['dimensionZ'])))
        voxelData = np.transpose(voxelData, (2,1, 0))
        voxelDataset = zeroGroup.create_dataset('VoxelData', voxelData.shape, dtype = int, compression="gzip")
        ...
        voxelDataset.attrs.create('nImages', int(reconRow['dimensionZ']))
        voxelDataset.attrs.create('scale', 1)
        voxelDataset.attrs.create('vertebraeIdentified', vertebraeIdentified)
        ...
        pn0Dataset = metadataGroup.create_dataset('pN0Seeds', pn0Data.shape, dtype = int)
        pn0Dataset[:] = pn0Data
