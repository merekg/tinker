...
MARKER_CENTER_OFFSET_MM = np.array([-0.0,0,0])

def overlay_volume_error(volume_points, nav_points, tx, volume_paths):
    perfect_points = get_perfect_markers(nav_points,tx)
    volume_points = volume_points.reshape((-1,6,3))
    perfect_points = perfect_points.T.reshape((-1,6,3))
    for path,p_vs, p_ps in zip(volume_paths,volume_points, perfect_points):
        print(path)
        volume, header = nrrd.read(path)
        # mark the volume centers
        for p_v in p_vs:
            p_v = p_v + [150,150,0]
            p_v /= 0.714286
            p_v = np.round(p_v).astype(int)
            print(*p_v)
            volume[p_v[0],p_v[1],p_v[2]] = 10000
        # mark the perfect centers
        for p_p in p_ps:
            p_p = p_p + [150,150,0]
            p_p /= 0.714286
            p_p = np.round(p_p).astype(int)
            print(*p_p)
            volume[p_p[0],p_p[1],p_p[2]] = 20000
        nrrd.write(os.path.splitext(path)[0]+"nrrd", volume, header)

def get_perfect_markers(A, T):

    # Transform the points A by the transformation T
    return np.matmul(T, np.vstack((A.T, np.ones(len(A.T[0])))))[:-1]
...
def order_cloud(cloud):
    ...
    ordered_cloud = []
    for marker in cloud:
        inds.append(get_marker_index(marker,cloud))
...
def get_camera_marker_centers(camera_paths):
    ...
    for camera_path in camera_paths:
        ...
        if(n_samples < MIN_CAMERA_SAMPLES):
...
def main():
    ...
    if testRun: # just check a current calibration
        ...
        overlay_volume_error(volume_marker_centers, camera_marker_centers, registration_matrix, volume_paths)
