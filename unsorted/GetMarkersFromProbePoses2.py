...
#PROBE_DEFINITION = np.array([[-159.44,-1.16,-0.71],
                             ...
                             #[-263.07,42.27,-0.71],
                             #[-304.47,-1.16,-0.71]])
PROBE_DEFINITION = np.array([[0.00, 0.00, 0.00], # this is the detector
                             [58.18, -34.13, 0.00],
                             [145.75, 0.00, 0.00],
                             [197.23, 58.27, 144.66],
...
#PROBE_DEFINITION = np.array([[-40.09,29.80,-11.47], # this is the PR
                             #[29.91,47.30,-11.47],
                             #[19.91,-20.20,-11.47],
                             #[-35.09,-10.20,-11.47]])
...
def main():
    ...
    # pull in the poses
    all_poses = np.fromfile(sys.argv[1],sep=' ').reshape((-1,7)).astype(float)
    print(all_poses.shape)

    for pose in all_poses:
        # The tool data comes in as [q0,qx,qy,qz,x,y,z]. Use this to determine the marker poses
        probe_rotation = R.from_quat(np.array([pose[1],pose[2],pose[3],pose[0]])).as_matrix()
        probe_translation = pose[4:]
        marker_poses = np.array([probe_rotation @ marker + probe_translation for marker in PROBE_DEFINITION])
