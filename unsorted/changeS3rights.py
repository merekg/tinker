...
while(True):
	...
	listingProc = subprocess.Popen(['aws', 's3api', 'list-objects-v2', '--bucket', 'nviewdatasets', '--starting-token', nextToken, '--max-items', str(batchSize), '--prefix', 'CT_scans/', '--query', '[Contents[].{Key: Key}, NextToken]'],stdout=subprocess.PIPE)
	...
	for p in range (0, processes):
	   ...
	   for k in range (0, batchSize):
		if (k % processes == p):
		   subcommand ='aws s3api put-object-acl --bucket nviewdatasets --key {} --grant-full-control emailaddress=cristian.atria@nviewmed.com --profile actualmed;'.format(keys[k]['Key'])
