...
DAT_FILE_PATH = '/home/nview/Desktop/Shalin/Data/'
...
def train(data, target, epochs):
	...
	model = createModel()
	model.summary()
	model_path = '/media/save/Shalin/SmartScrolling/Models/model_cubic' + str(epochs)+'.h5'
	...
	model = keras.models.load_model('/media/save/Shalin/SmartScrolling/Models/model_cubic' + str(epochs)+'.h5', compile = False)
	predictions = model.predict(data)
	return predictions

def test(data):
	model = keras.models.load_model('/media/save/Shalin/SmartScrolling/Models/model_cubic.h5', compile = False)
	predictions = model.predict(data)
	return predictions
...
def getPredictions():
	data = np.load('/media/save/Shalin/SmartScrolling/Data/CubicDataToTest.npy')
	model = keras.models.load_model('/media/save/Shalin/SmartScrolling/BestModels/model_cubic250.h5')
	predictions = model.predict(data)
...
# np.save('/home/nview/Desktop/Shalin/data.npy', data)
epochs = [150, 200, 250, 300]

data, target = loadDataset('/media/save/Shalin/SmartScrolling/Data/augmentedCubicTrainData.npy', '/media/save/Shalin/SmartScrolling/CubicFiles/augmentedTrainLabelsCubic.csv' )
# np.savetxt("targets.csv", target, delimiter=",", fmt='%f')
augmentedTracker = np.genfromtxt('/media/save/Shalin/SmartScrolling/CubicFiles/augmentedTracker.csv', delimiter = ',')
data, target, augmentedTracker = shuffle(data, target, augmentedTracker)
print(data.shape, target.shape)
np.savetxt("/media/save/Shalin/SmartScrolling/CubicFiles/augmentedTrackerShuffled.csv", augmentedTracker, delimiter=",", fmt='%s')

# testData, testTarget = loadDataset('/media/save/Shalin/SmartScrolling/Data/cubicTestData.npy', '/media/save/Shalin/SmartScrolling/Files/testLabelsCubic.csv' )
