...
def compute_rigid_transform(camera_points, sensor_points):

    # Verify that: the shape of camera_points and sensor_points match, each have 3 rows
    assert camera_points.shape[0] == 3
    assert sensor_points.shape[0] == 3

    # Compute the centroid of each matrix
    Ac = np.mean(camera_points, axis=1)[:,None]
    Bc = np.mean(sensor_points, axis=1)[:,None]

    # Compute covariance of centered matricies and find rotation using SVD
    H = np.matmul(camera_points - Ac, (sensor_points - Bc).T)
...
def get_volume_marker_centers(volume_paths):
    ...
    for volume_path in volume_paths:
        ...
        sensor_poses.append(order_cloud(current_volume_markers))

    return np.array(sensor_poses).reshape((-1,3))

def apply_tx_to_points(Tx, points):

    tx_p = np.zeros((points.shape[0], 3))

    for i in range(tx_p.shape[0]):
        # The tool data comes in as [x,y,z,qx,qy,qz,qw]. Use this to determine the marker poses
        panel_orientation = R.from_quat(np.array([points[i][1],points[i][2],points[i][3],points[i][0]])).as_matrix()
        panel_translation = points[i][4:]
        D = to_transform_matrix(panel_orientation,panel_translation)
        tx_p[i] = apply_rigid_transform(np.array([-296.3/2,-296.3/2,0]), Tx @ D)
...
def main():
    ...
    # camera points
    camera_marker_centers = np.fromfile(sys.argv[1],sep=' ').reshape((-1,7))
    n_camera_markers = len(camera_marker_centers)
    print("Found", n_camera_markers, "positions from camera.")

    # sensor points
    sensor_poses = np.fromfile(sys.argv[2],sep=' ').reshape((-1,3))
    n_sensor_poses = len(sensor_poses)
    print("Found", n_sensor_poses, "positions from sensors.")

    # navigation calibration
    cal = np.fromfile(sys.argv[3],sep=' ').reshape((4,4))
    camera_marker_centers = apply_tx_to_points(cal, camera_marker_centers)

    # Check that the number of markers is the same for each cloud
    assert n_camera_markers is n_sensor_poses

    # Compute rigid transform
    rotation, translation = compute_rigid_transform(camera_marker_centers.T, sensor_poses.T)
    ...
    error = check_transformation(camera_marker_centers.T, sensor_poses.T, registration_matrix)
