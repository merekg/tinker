...
PM_FILE_NAME = "PM.cal"
TECHNIQUE_CALIBRATION_COMMAND = "python /opt/rt3d/bin/TechniqueCalibration.py"
...
# ====== Functions =====================================================================
def printHelp():
    print("Use: python Calibrate.py outputFolder [offset technique signalInAir motion]")
...
def runSignalInAirCalibration(signalInAirOutDir):
    ...
    print('\a')
    raw_input("Scan over the following image to verify image quality. Press enter when ready to continue...")
...
def runMotionCalibration(motionOutDir, seedPath):
    ...
    ctPath = ""
    while(True):
        ctPath = raw_input("Please enter the location of the CT ground truth file: ")
    ...
    raw_input("Press enter when this is done and you are ready to continue...\n")
...
def main():
    ...
    outputFolderPath, calibrationInputFlags = parseInputs(sys.argv[1:])
    openLogFile(outputFolderPath)
    ...
    if(calibrationInputFlags.get("offset")):
        offsetOutDir = createDirectory(outputFolderPath, str(datetime.now()).translate(None, INVALID_FILE_PATH_CHARACTERS) + OFFSET_FOLDER_NAME)
        runOffsetCalibration(offsetOutDir)
    if(calibrationInputFlags.get("technique")):
        techniqueOutDir = createDirectory(outputFolderPath, str(datetime.now()).translate(None, INVALID_FILE_PATH_CHARACTERS) + TECHNIQUE_FOLDER_NAME)
        runTechniqueCalibration(techniqueOutDir)
    if(calibrationInputFlags.get("signalInAir")):
        signalInAirOutDir = createDirectory(outputFolderPath, str(datetime.now()).translate(None, INVALID_FILE_PATH_CHARACTERS) + SIA_FOLDER_NAME)
        runSignalInAirCalibration(signalInAirOutDir)
    if(calibrationInputFlags.get("motion")):
        motionOutDir = createDirectory(outputFolderPath, str(datetime.now()).translate(None, INVALID_FILE_PATH_CHARACTERS) + MOTION_FOLDER_NAME)
        runMotionCalibration(motionOutDir, calibrationInputFlags['motionSeed'])
