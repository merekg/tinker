...
def predictAndUploadAws(model, predictReconVolume, predictFileName):
    ...
    subprocess.check_output(['aws', 's3', 'cp', predictFileName,'s3://nviewdatasets/TestSet_PredictedRecons/'])
...
def fetchTestImage(name):
    ...
    subprocess.check_output(['aws', 's3api', 'get-object', '--bucket', 'nviewdatasets', '--key', testImageReconKey,testImageRecon])
