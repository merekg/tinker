...
def fetchTestImage(name):
    ...
    subprocess.check_output(['aws', 's3api', 'get-object', '--bucket', 'nviewdatasets', '--key', good_image_gt_key, good_image_truth])
    subprocess.check_output(['aws', 's3api', 'get-object', '--bucket', 'nviewdatasets', '--key', good_image_recon_key,good_image_train])
...
def generateValidateRecon(batchSize, validationSize):
    ...
    while (True):
        ...
        objectlistCMD = "aws s3api list-objects-v2 --bucket nviewdatasets --start-after " + lastValidationFile + " --max-items " + str(batchSize+validationSize) + " --prefix " + reconBucket +" --query '[Contents[].{Key: Key}]'"
        ...
        if len(keys) < batchSize+validationSize:
            ...
            objectlistCMD = "aws s3api list-objects-v2 --bucket nviewdatasets  --max-items " + str(batchSize+validationSize-len(keys)) + " --prefix " + reconBucket +" --query '[Contents[].{Key: Key}]'"
            #Find the name of the file
            objectsInStudy = subprocess.Popen(objectlistCMD, shell=True, stdout=subprocess.PIPE)
            std_out, std_error = objectsInStudy.communicate()
            data = json.loads(std_out)
        ...
        for k in range(batchSize, len(keys)): #keys[0] = 1st Batch File, key[batchSize] = 1st Validation File
            ...
            subprocess.check_output(['aws', 's3api', 'get-object', '--bucket', 'nviewdatasets', '--key', gtkey, gt])
            subprocess.check_output(['aws', 's3api', 'get-object', '--bucket', 'nviewdatasets', '--key', reconKey, recon])
...
#findLengthCMD = "aws s3api list-objects --bucket nviewdatasets --prefix Recons/ --output json --query 'length(Contents[])'"
